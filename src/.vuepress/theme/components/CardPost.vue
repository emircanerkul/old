    ...
    <article class="card box-default" :class="{ 'card--cover-top': isCoverPosition('top') }">
      <div class="card__box1" :class="`card__box1--${cover}`">
        <header>
          <div class="card-category">
            <router-link class="card-category__link" :to="getCurrentCategory.path"
            ...
            <router-link :to="item.path" :aria-labelledby="`sr-link-post-${item.key}`">
              <responsive-picture :coverName="item.coverName">
                <img class="card-cover__image card-cover__image--top cover"
                  :src="`${item.coverName},w_${lastBreakpoint}.${extension}`" :title="item.coverAlt"
                  :alt="item.coverAlt">
              </responsive-picture>
            </router-link>
          </div>
      ...
      <div class="card__box2" :class="`card__box2--${cover}`" v-if="item.cover && cover && !isCoverPosition('top')">
        <router-link :to="item.path">
          <img class="card-cover__image card-cover__image--side cover" :src="getSideImage(item.coverName)"
            :alt="item.coverAlt">
        </router-link>
      </div>
    </article>

    <ins v-if="(index + 1) % 3 == 0" class="adsbygoogle" style="display:block; margin-bottom: 15px; border-radius: 10px; overflow: hidden;" data-ad-format="fluid"
      data-ad-layout-key="-fc-33+cf-nw-en" data-ad-client="ca-pub-2293506510878791" data-ad-slot="1137554367"></ins>
  </div>
</template>
